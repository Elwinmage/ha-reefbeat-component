...
class ReefBeatAPI:
    ...
    async def _http_get(self, client, source):
        ...
        if self._secure and (int(now - self._auth_date) > 2700):
            await self.connect()
        r = await client.get(
            self._base_url + source.value["name"],
            timeout=DEFAULT_TIMEOUT,
            headers=self._header,
        )
        # 403 patch for rswave
        if r.status_code == 200 or (
            r.status_code == 503 and source.value["name"] == "/"
        ):
            response = r.json()
            ...
            _LOGGER.error(
                "Can not get data: %s from %s" % (source.value["name"], self.ip)
            )
            return False
    ...
    async def _call_url(self, client, source):
        ...
        while status_ok is False and error_count < HTTP_MAX_RETRY:
            try:
                ...
                _LOGGER.debug(
                    ...
                    % (source.value["name"], error_count, HTTP_MAX_RETRY)
                )
                _LOGGER.debug(e)
            if not status_ok:
        ...
        if not status_ok:
            _LOGGER.error(
                ...
                % (self.ip, source.value["name"], HTTP_MAX_RETRY)
            )
            self._in_error = True
    ...
    async def get_initial_data(self):
        ...
        async with httpx.AsyncClient(verify=False) as client:
            await asyncio.gather(
                *[
                    self._call_url(client, sources[source])
    ...
    def _get_data(self, data_name, is_None_possible=False):
        ...
        if len(res) == 0:
            if is_None_possible is False:
                _LOGGER.error(
                    "reefbeat.get_data('%s') %s" % (data_name, self._base_url)
                )
                _LOGGER.error("%s" % self.data)
            else:
                return None
        ...
        while status_ok is False and error_count <= HTTP_MAX_RETRY:
            try:
                if method == "post":
                    ...
                    r = httpx.delete(
                        url, verify=False, timeout=DEFAULT_TIMEOUT, headers=self._header
                    )
                status_ok = (
                ...
                _LOGGER.debug(
                    ...
                    % (method, payload, url, error_count, HTTP_MAX_RETRY)
                )
                _LOGGER.debug(e)
...
class ReefLedAPI(ReefBeatAPI):
    ...
    def __init__(
        self, ip, live_config_update, hw, intensity_compensation=False
    ) -> None:
        ...
        try:
            status_code = httpx.get(
                "http://" + ip + "/dashboard", verify=False
            ).status_code
        except Exception:
            pass
        if status_code != 200:
        ...
        try:
            status_code = httpx.get(
                "http://" + ip + "/preset_name", verify=False
            ).status_code
        except Exception:
            pass
        if status_code == 200:
            self.data["sources"].insert(
                len(self.data["sources"]),
                {"name": "/preset_name", "type": "config", "data": ""},
            )
        else:
            for day in range(1, 8):
                self.data["sources"].insert(
                    len(self.data["sources"]),
                    {"name": "/preset_name/" + str(day), "type": "config", "data": ""},
                )
        self.data["sources"].insert(
            len(self.data["sources"]), {"name": "/manual", "type": "data", "data": ""}
        )
        self.data["sources"].insert(
            len(self.data["sources"]),
            {"name": "/acclimation", "type": "config", "data": ""},
        )
        self.data["sources"].insert(
            len(self.data["sources"]),
            {"name": "/moonphase", "type": "config", "data": ""},
        )
        for day in range(1, 8):
            self.data["sources"].insert(
                len(self.data["sources"]),
                {"name": "/auto/" + str(day), "type": "config", "data": ""},
            )
            self.data["sources"].insert(
                len(self.data["sources"]),
                {"name": "/clouds/" + str(day), "type": "config", "data": ""},
            )
    ...
    async def get_initial_data(self):
        ...
        # Kelvin conversion
        led_params = self.get_data(
            '$.local.leds_conv[?(@.name=="' + self._model + '")]'
        )
        _LOGGER.debug("LEDS_PARAMS: %s" % led_params)
        if len(led_params["kelvin"]) > 0:
            self._kelvin_to_wb = np.poly1d(
                np.polyfit(
                    np.array(led_params["kelvin"]),
                    np.array(led_params["white_blue"]),
                    4,
                )
            )
        ...
        if self._must_compensate_intensity:
            led_params = self.get_data(
                '$.local.leds_intensity_compensation[?(@.name=="' + self._model + '")]',
                True,
            )
    ...
    def kelvin_to_white_and_blue(self, kelvin, intensity=100):
        ...
        if self._intensity_compensation is not None and kelvin >= 12000:
            intensity_compensation_factor = (
                ...
                / self._intensity_compensation(wb)
            )
        ...
        _LOGGER.debug(
            "Kelvin to white and blue: %s (wb=%d) whit compensation %s"
            % (res, wb, intensity_compensation_factor)
        )
        return res
    ...
    def update_light_ki(self):
        ...
        data = {
            "kelvin": int(self.get_data(LED_KELVIN_INTERNAL_NAME)),
            "intensity": int(self.get_data(LED_INTENSITY_INTERNAL_NAME)),
        }
    ...
    def force_status_update(self, state=False):
        if state:
            ...
            if (
                self.get_data(LED_WHITE_INTERNAL_NAME) > 0
                or self.get_data(LED_BLUE_INTERNAL_NAME) > 0
                or self.get_data(LED_MOON_INTERNAL_NAME) > 0
            ):
                self.data["local"]["status"] = True
            else:
...
class ReefDoseAPI(ReefBeatAPI):
    ...
    def __init__(self, ip, live_config_update, heads_nb) -> None:
        ...
        self.data["sources"].insert(
            len(self.data["sources"]),
            {"name": "/device-settings", "type": "config", "data": ""},
        )
        self.data["sources"].insert(
            len(self.data["sources"]),
            {"name": "/dosing-queue", "type": "data", "data": ""},
        )
        if self._heads_nb == 2:
            ...
            _LOGGER.error(
                "redsea.reefbeat.ReefDoseAPI.__init__() unkown head number: %d"
                % self._heads_nb
            )
    ...
    async def calibration(self, action, head, param):
        await self._http_send(
            self._base_url + "/head/" + str(head) + "/" + action, param
        )
        uid = self.get_data(
            "$.sources[?(@.name=='/head/"
            + str(head)
    ...
    async def press(self, action, head):
        if head is not None:
            ...
            await self._http_send(
                self._base_url + "/head/" + str(head) + "/" + action, payload
            )
        else:
            payload = {}
            await self._http_send(self._base_url + "/" + action, payload)
...
class ReefWaveAPI(ReefBeatAPI):
    ...
    def __init__(self, ip, live_config_update) -> None:
        ...
        self.data["sources"].insert(
            len(self.data["sources"]),
            {"name": "/feeding/schedule", "type": "config", "data": ""},
        )
        self.data["sources"].insert(
            len(self.data["sources"]), {"name": "/auto", "type": "data", "data": ""}
        )
        self.data["sources"].insert(
            len(self.data["sources"]),
            {"name": "/device-settings", "type": "config", "data": ""},
