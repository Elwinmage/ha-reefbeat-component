...
with open(const_file) as f:
   entity_domains = list(map(lambda x: x.replace(r'Platform.','').lower() ,re.findall(r'Platform\.[A-Z\_]*', f.read())))
   entity_domains.sort()
...
for file in os.listdir(translations_path):
    if file.endswith(".json"):
        with open(translations_path+"/"+file) as f:
            langs+=[{"lang":file.replace(".json",""),"data":json.load(f),"translations_keys":[]}]

for entity_domain in entity_domains:
    #Get keys in code
    with open("../"+entity_domain+".py") as f:
        keys_in_code += list(map(lambda x: entity_domain+'.'+x.replace(r'translation_key=','').replace('\"',''),re.findall(r'translation_key=\"[a-zA-Z0-9\-\_]*\"',f.read())))
    for lang in langs:
       lang_trans=list(map(lambda x: entity_domain+"."+x,lang['data']['entity'][entity_domain].keys()))
       lang_trans.sort()
...
for lang in langs:
   ...
   if len(needed) > 0:
      modifs+=len(needed)
      print ("["+str(len(needed))+"] Needed in "+Fore.CYAN+path+lang['lang']+".json")
      for need in needed:
   ...
   if len(no_more_needed) > 0:
      ...
      print ("["+str(len(no_more_needed))+"] No More Needed in "+Fore.CYAN+path+lang['lang']+".json")
