...
from custom_components.redsea.const import ATO_BUZZER_ENABLED_INTERNAL_NAME, DOMAIN
...
async def test_ato_buzzer_switch_toggle_pushes_configuration() -> None:
    """Toggling writes the cache then PUTs `/configuration`."""
    device = _AtoDevice()
    ...
    assert device.refreshed == ["/configuration"]
