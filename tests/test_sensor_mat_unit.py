...
@pytest.mark.asyncio
async def test_async_setup_entry_mat_branch(monkeypatch: Any, hass: Any) -> None:
    class _Mat(_FakeCoordinator):
        pass

    monkeypatch.setattr(sensor_platform, "ReefMatCoordinator", _Mat)
    ...
    one = sensor_platform.ReefBeatSensorEntityDescription(
        key="mat", translation_key="mat", value_fn=lambda _: 1
    )
    ...
    monkeypatch.setattr(sensor_platform, "MAT_SENSORS", (one,))
    ...
    hass.data.setdefault(DOMAIN, {})[entry.entry_id] = _Mat(hass=hass)
